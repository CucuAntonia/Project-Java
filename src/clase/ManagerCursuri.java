...
public class ManagerCursuri {
    ...
    ///Studentii sa poata fi usor grupati dupa numarul grupei din care fac
    //parte.
    void GrupareStudentiDupaGrupa(List<Student>studenti) {
        HashMap<Integer, List<Student>> grupareStudentiDupaNumarulGrupei = new HashMap<>();
        // Set<Student> set_stud=Curs.studenti;
        //List <Student> stud = new ArrayList<Student>(set_stud);

        for (Student s :studenti ) {
            int element = s.grupa;
            // ArrayList<Student> aparitie = grupareStudentiDupaNumarulGrupei.get(element);
            if (grupareStudentiDupaNumarulGrupei.get(element) != null) {
                List<Student> stud=new ArrayList<>();
                stud=grupareStudentiDupaNumarulGrupei.get(element);
                stud.add(s);
                grupareStudentiDupaNumarulGrupei.put(element,stud);
            }
            else
            {
                grupareStudentiDupaNumarulGrupei.put(element,s);
            }


        }

        for (Map.Entry<Integer, List<Student>> entry : grupareStudentiDupaNumarulGrupei.entrySet()) {
            Integer key = entry.getKey();
            List<Student> value = entry.getValue();
            for(Student s:value)
                System.out.println("Studentul" + s + " face parte din grupa " + key);
        }
    }
